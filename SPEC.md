...
### 4.3 Starter Entity/Relationship Taxonomy (proposal — confirm before implementation, see Open Question 3)

Entity types: `Person`, `Username`, `Email`, `PhoneNumber`, `Domain`, `IPAddress`, `OnlineAccount`, `Organization`, `Location`, `Document`, `Image`, `Vehicle`, `Custom` (escape hatch).

Relationship types: `HasAccount`, `Owns`, `AssociatedWith`, `LocatedAt`, `MemberOf`, `ResolvesTo`, `Mentions`, `RelatedTo` (generic catch-all).
...
1. **Case portability vs. keychain-tied encryption.** The encryption key lives in the OS keychain of the machine that created the case, which conflicts with the goal of handing a case file to another investigator/machine/jurisdiction. Need a defined export/handoff mode (e.g. passphrase-derived export encryption, distinct from at-rest keychain encryption).
2. **Plugin signing authority.** Who holds the signing key in v1 — a single hardcoded public key for first-party plugins only? What's the eventual process if/when third-party plugins are considered?
3. **Confirm the starter entity/relationship taxonomy** (Section 4.3) before M1/M2 implementation — is it complete enough, and are the names right?
4. **Data retention/redaction.** The audit log is append-only by design, but investigators may need to redact a wrongly-collected fact (e.g. for legal reasons). Need a defined redaction mechanism that doesn't undermine the tamper-evidence goal (e.g. crypto-shredding a specific fact vs. true deletion).
